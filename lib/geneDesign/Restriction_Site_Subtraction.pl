...
if ($config{HELP}) {
	...
	Usage examples:
	perl Restriction_Site_Subtraction.pl -i Test_YAR000W.FASTA -o 13 -s BsaI:BsmBI 
  -w YAR000W_recode.FASTA -t 10 
...
foreach my $org (@ORGSDO)
{
	...
	foreach my $seqkey (sort {$a cmp $b} keys %$nucseq)
	{
		...
		$Error4 = "I was unable to remove @fail_enz after $iter iterations.\n" if @fail_enz;
		$Error0 = "I successfully removed @success_enz from your sequence.\n" if @success_enz;
		$Error5 = "There were no instances of @none_enz present in your sequence.\n" if @none_enz;
		$Error7 = "I was unable to remove some instances of @semifail_enz after $iter iterations.\n" if @semifail_enz;
...
#
#		"
		print "$Error4 $Error0 $Error7 $Error8"
	}
